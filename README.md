...
<!-- =========================================================
     ND BURGS — CORREÇÕES SOLICITADAS 07/09/2026
     1) Remove a faixa superior antiga com ND BURGS
     2) Corrige os produtos de "CONTINUAR COMPRANDO"
     3) Troca ADICIONAR -> COMPRAR nos produtos
     4) Após adicionar: COMPRAR -> ADICIONADO + verde
     ========================================================= -->
<style id="nd-fix-20260907">
  /* Remove a faixa superior que exibia ND BURGS no topo */
  .nd-v3-strip{display:none!important}

  /* Estado visual do botão após adicionar */
  .produto .btn-add.nd-buy-added,
  .produto .nd-fx-add.nd-buy-added{
    background:linear-gradient(135deg,#19b95b,#25d366)!important;
    color:#fff!important;
    border-color:transparent!important;
    box-shadow:0 8px 22px rgba(37,211,102,.25)!important;
  }
  .produto .btn-add.nd-buy-added:hover,
  .produto .nd-fx-add.nd-buy-added:hover{
    background:linear-gradient(135deg,#18aa54,#2be878)!important;
    filter:brightness(1.04)!important;
  }
  .nd-v3-upsell .nd-v3-up button.nd-buy-added{
    background:#25d366!important;
    color:#07140b!important;
  }
</style>

<script id="nd-fix-20260907">
(function(){
  'use strict';

  const DIRECT_ADD_RE=/\badicionar\s*\(/i;
  const CUSTOM_RE=/abrir(?:Combo)?Personalizacao\s*\(|abrirPersonalizacao\s*\(/i;

  function normalize(v){
    return String(v||'').replace(/\s+/g,' ').trim();
  }

  function setBuyLabel(btn, added){
    if(!btn) return;
    if(added){
      btn.textContent='ADICIONADO';
      btn.classList.add('nd-buy-added');
      btn.setAttribute('aria-label','Produto adicionado ao carrinho');
      btn.dataset.ndBuyState='added';
    }else if(!btn.dataset.ndBuyState || btn.dataset.ndBuyState==='buy'){
      btn.textContent='COMPRAR';
      btn.classList.remove('nd-buy-added');
      btn.setAttribute('aria-label','Comprar produto');
      btn.dataset.ndBuyState='buy';
    }
  }

  function findProductCardByName(name){
    const target=normalize(name).toLocaleUpperCase('pt-BR');
    return [...document.querySelectorAll('.produto')].find(card=>{
      const h=card.querySelector('h3');
      return normalize(h?.textContent).toLocaleUpperCase('pt-BR')===target;
    })||null;
  }
  ...
  function markAdded(name){
    ...
    if(!card) return;
    card.querySelectorAll('.btn-add,.nd-fx-add').forEach(btn=>setBuyLabel(btn,true));
  }

  function normalizeProductButtons(root=document){
    const scope=root.querySelectorAll ? root : document;
    scope.querySelectorAll?.('.produto .btn-add,.produto .nd-fx-add').forEach(btn=>{
      if(btn.dataset.ndBuyState==='added') return;
      setBuyLabel(btn,false);
    });
  }

  /*
   * Corrige a área de sugestões:
   * - produtos simples são adicionados diretamente pelo onclick original;
   * - produtos que exigem personalização continuam abrindo a personalização,
   *   sem quebrar o fluxo original.
   */
  window.ndAdicionarSugestao=function(card){
    ...
    try{
      if(onclick.trim()){
        /* Executa exatamente a ação original do produto. */
  ...
  /*
   * Recria o clique das sugestões para não depender de botão escondido,
   * hover ou handlers adicionados por outras rodadas do site.
   */
  function patchSuggestions(){
    const grid=document.getElementById('suggestionsGrid');
    if(!grid || grid.dataset.ndFixSuggestions==='1') return;
    grid.dataset.ndFixSuggestions='1';
    ...
    grid.addEventListener('click',function(e){
      ...
      const index=Number(b.dataset.sug);
      const cards=[...document.querySelectorAll('.produto:not(.search-hidden)')]
        .filter(c=>!c.closest('#checkout') && c.querySelector('.btn-add'));

      /*
       * A lista original é aleatória e pode mudar depois do render.
       * O botão guarda o nome do produto para encontrar o card correto.
       */
      const nome=b.dataset.sugName || normalize(b.closest('.suggestion-card')?.querySelector('strong')?.textContent);
      let card=cards.find(c=>normalize(c.querySelector('h3')?.textContent)===nome);

      if(!card && Number.isInteger(index)) card=cards[index];

      if(card){
        window.ndAdicionarSugestao(card);
        setTimeout(()=>{ if(typeof window.mostrarSugestoes==='function') window.mostrarSugestoes(); },180);
      }
    },true);
  }

  /*
   * Substitui o renderizador de sugestões existente por uma versão que
   * guarda o nome no botão e usa o mesmo fluxo de compra do produto.
   */
  function patchMostrarSugestoes(){
    if(typeof window.mostrarSugestoes!=='function' || window.mostrarSugestoes.__ndFix20260907) return;

    const original=window.mostrarSugestoes;
    ...
    window.mostrarSugestoes=function(){
      ...
      const grid=document.getElementById('suggestionsGrid');
      if(grid){
        grid.querySelectorAll('button[data-sug]').forEach(b=>{
          const name=normalize(b.closest('.suggestion-card')?.querySelector('strong')?.textContent);
          if(name) b.dataset.sugName=name;
    ...
    window.mostrarSugestoes.__ndFix20260907=true;
  }

  /*
   * Depois de qualquer adicionar(), marca o produto correspondente como
   * ADICIONADO. Isso também cobre combos/personalizações que concluem o
   * fluxo chamando adicionar() no final.
   */
  function patchAdicionar(){
    if(typeof window.adicionar!=='function' || window.adicionar.__ndFix20260907) return;

    const old=window.adicionar;
    window.adicionar=function(nome,preco){
      const result=old.apply(this,arguments);
      setTimeout(()=>markAdded(nome),30);
      return result;
    };
    window.adicionar.__ndFix20260907=true;
  }

  function syncButtonStatesFromCart(){
    const cart=Array.isArray(window.carrinho)?window.carrinho:[];
    const names=new Set(cart.map(x=>normalize(x?.nome).toLocaleUpperCase('pt-BR')));
    document.querySelectorAll('.produto').forEach(card=>{
      const name=normalize(card.querySelector('h3')?.textContent).toLocaleUpperCase('pt-BR');
      const added=names.has(name);
      card.querySelectorAll('.btn-add,.nd-fx-add').forEach(btn=>{
        if(added) setBuyLabel(btn,true);
        else if(btn.dataset.ndBuyState!=='added') setBuyLabel(btn,false);
      });
    });
  }

  function init(){
    normalizeProductButtons();
    patchAdicionar();
    patchMostrarSugestoes();
    patchSuggestions();
    syncButtonStatesFromCart();
  }
  ...
  [300,800,1500,2500,4000].forEach(ms=>setTimeout(init,ms));
})();
</script>
